      ...
      LocalStoreSlaveTransport.prototype.listen = function (url, keys) {
        ...
        this.statusChangeChecker = ughbind(function (ev) {
          if (ev.key == this.keys.msg) {
            ...
            this.emit('message', storage.getItem(this.keys.msg), {
...
function ajax(options) {
    ...
    if (xmlHttp.readyState == 4) {
      try {
        if (xmlHttp.status == 200) {
          ...
          options.error(JSON.parse(xmlHttp.responseText))
        }
      } catch (e) {
        options.error({
          "Status": "failed",
  ...
  if (options.headers) {
    ...
    for (var i = 0; i < headerKeys.length; i++) {
      ...
      var headerValue = options.headers[headerKeys];
...
window.Authme = function (apiKey) {
  return {
    getLoginToken: function (email, referenceId, callback, endPoint) {
      const l = function (email, referenceId, callback, tries) {
        ...
        ajax({
          ...
          success: function (response) {
            ...
            if (response.Status != "auth_initiated") {
              callback(response);
              return;
            }

            const sub = new NchanSubscriber("https://ws.authme.authme.host/ws/sub/" + response.ReferenceId.replace(/\-/g, ""), opt = {
